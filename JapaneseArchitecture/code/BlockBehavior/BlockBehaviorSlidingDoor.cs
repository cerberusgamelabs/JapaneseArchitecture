...
namespace JapaneseArchitecture.code.BlockBehavior {
    public class BlockBehaviorSlidingDoor : StrongBlockBehavior, IMultiBlockColSelBoxes, IMultiBlockBlockProperties {
        ...
        public BlockBehaviorSlidingDoor(Block block) : base(block) {
            ...
            openCollisionSel = block.Attributes["openCollisionSel"].AsArray();
        ...
        public override void Activate(IWorldAccessor world, Caller caller, BlockSelection blockSel, ITreeAttribute activationArgs, ref EnumHandling handled) {
            var beh = world.BlockAccessor.GetBlockEntity(blockSel.Position)?.GetBehavior<BEBehaviorSlidingDoor>();
        ...
        public static BEBehaviorSlidingDoor getDoorAt(IWorldAccessor world, BlockPos pos) {
            var door = world.BlockAccessor.GetBlockEntity(pos)?.GetBehavior<BEBehaviorSlidingDoor>();
            if (door != null) return door;

            var blockMb = world.BlockAccessor.GetBlock(pos) as BlockMultiblock;
            if (blockMb != null) {
                door = world.BlockAccessor.GetBlockEntity(pos.AddCopy(blockMb.OffsetInv))?.GetBehavior<BEBehaviorSlidingDoor>();
                return door;
            }
        ...
        public override bool CanPlaceBlock(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel, ref EnumHandling handling, ref string failureCode) {
            var rotRad = BEBehaviorSlidingDoor.getRotateYRad(byPlayer, blockSel);
            bool blocked = false;
        ...
        public bool placeDoor(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, BlockPos pos, IBlockAccessor ba) {
            ...
            var bh = ba.GetBlockEntity(pos)?.GetBehavior<BEBehaviorSlidingDoor>();
            ...
            for (int dx = 0; dx < width; dx++) {
                for (int dy = 0; dy < height; dy++) {
                    for (int dz = 0; dz < width; dz++) {
                        var offset = BEBehaviorSlidingDoor.getAdjacentOffset(dx, dz, dy, yRotRad, invertHandle);
        ...
        public override Cuboidf[] GetCollisionBoxes(IBlockAccessor blockAccessor, BlockPos pos, ref EnumHandling handled) {
            ...
            return blockAccessor.GetBlockEntity(pos)?.GetBehavior<BEBehaviorSlidingDoor>()?.ColSelBoxes ?? null;
        }
        ...
        public override float GetLiquidBarrierHeightOnSide(BlockFacing face, BlockPos pos, ref EnumHandling handled) {
            handled = EnumHandling.PreventDefault;
            var beh = block.GetBEBehavior<BEBehaviorSlidingDoor>(pos);
        ...
        public float MBGetLiquidBarrierHeightOnSide(BlockFacing face, BlockPos pos, Vec3i offset) {
            var beh = block.GetBEBehavior<BEBehaviorSlidingDoor>(pos.AddCopy(offset.X, offset.Y, offset.Z));
            if (beh == null) return 0f;
            if (!beh.IsSideSolid(face)) return 0f;
